$(document).ready(function(){
    ...
    // $('input.title-input').on('blur', function () {
        ...
        // $.form.load('admin/Sta/save', {name: this.name, value: this.value}, 'POST', function (ret) {
            // return false;
        // });
    // });
	...
	$('img[nc_type="flex"]').click(function(){
		...
		if(status == 'open'){
			...
			//ajax
			$.ajax({
				url: '/admin/Sta/ajaxGetArea?act=area&op=area&ajax=1&area_parent_id='+id,
				dataType: 'json',
				success: function(data){
					...
					for(var i = 0; i < data.length; i++){
                        ...
                        src +="<td style='width:10px;'>";
                        // src +="<input class='list-check-box' value='"+data[i].ids+"' type='checkbox'/>";
						...
						src += "</td>";
						// src += "<td class='list-table-sort-td'>";
						//排序
						...
						// src += "<span>"+data[i].area_sort+" </span>";
						//名称
						src += "<td style='width:200px;' class='text-center nowrap'>";
						...
						src += "<span>"+data[i].area_name+" </span>";
						src += "</td>";

                        // 学员数+师资数
                         src += "<td class=' nowrap'>";
                             src += "<span>站点数：<a style='color:red;' href='"+data[i].d_url+"&zone_id="+data[i].area_id+"'>"+data[i].total+"</a>个</span>；";
                             src += "<span>主管用户数：<a style='color:red;' href='"+data[i].u_url+"&aid="+data[i].area_id+"'>"+data[i].managerUNum+"</a>个</span>；";
                             src += "<span>服务商用户数：<a style='color:red;' href='"+data[i].u_url+"&aid="+data[i].area_id+"'>"+data[i].serverUNum+"</a>个</span>；";
                             src += "<span>企业用户数：<a style='color:red;' href='"+data[i].u_url+"&aid="+data[i].area_id+"'>"+data[i].enterUNum+"</a>个</span>；";
                             src += "<span>电商扶贫用户数：<a style='color:red;' href='"+data[i].u_url+"&aid="+data[i].area_id+"'>"+data[i].dsUNum+"</a>个</span>；";
                             src += "<span>建档立卡用户数：<a style='color:red;' href='"+data[i].u_url+"&aid="+data[i].area_id+"'>"+data[i].jdUNum+"</a>个</span>；";
                         src += "</td>";
      ...
      //                   src += "</td>";
					...
					$.getScript("static/admin/jquery.area1030.js");
